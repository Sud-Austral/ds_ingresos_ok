...
```{r}
options(scipen=999)
casen2017_algarrobo <- filter(casen2017, casen2017$comuna == "Algarrobo")
...
# write_xlsx(casen2017_algarrobo_head_sin_na,"casen2017_algarrobo_head_sin_na.xlsx")
```
...
```{r}
repetidos2 <- casen2017_algarrobo_head_sin_na
...
repetidos2 <- merge(x=casen_fh, y=tabla, by = "folio", all.x = T)
# rr <- repetidos2[!duplicated(repetidos2$folio), ]
rr2 <- repetidos2[!duplicated(repetidos2$folio), ]
rr <- repetidos2
head(rr,10)
```
```{r}
...
casen2020_algarrobo <- casen2020_algarrobo[,c("comuna","folio","ytot", "ytotcor","ytoth","yaimh","yaimcorh","ytotcorh")]
casen2020_algarrobo
...
casen_fh <- casen2020_algarrobo_head_sin_na[,c("comuna","folio","ytoth","ytotcorh","yaimcorh","yaimh")]
casen_fh$folio <- as.character(casen_fh$folio)
repetidos2 <- merge(x=casen_fh, y=tabla, by = "folio", all.x = T)
rr <- repetidos2[!duplicated(repetidos2$folio), ]
rr
...
```{r}
rr$multi <- rr$ytotcorh/rr$freq_folio
rr$multi_sin_imput <- (rr$ytotcorh/rr$freq_folio)-rr$yaimcorh
rr
```
