...
# saveRDS(tabla_2017,"C:/Users/enamo/Desktop/Shiny-R/Casen_en_pandemia_2020/casen/casen_2017_c2.rds")
# 
...
# saveRDS(tabla_2015,"C:/Users/enamo/Desktop/Shiny-R/Casen_en_pandemia_2020/casen/casen_2015_c2.rds")
# 
...
# saveRDS(tabla_2013,"C:/Users/enamo/Desktop/Shiny-R/Casen_en_pandemia_2020/casen/casen_2013_c2.rds")
# 
...
# saveRDS(tabla_2011,"C:/Users/enamo/Desktop/Shiny-R/Casen_en_pandemia_2020/casen/casen_2011_c2.rds")
...
## Ingreso familias 2020

No se despliega informacion 2020 parcial por razones de tiempo de ejecucion, sin embargo en el apartado tabla de familias 2011-2020 esta incluida
...
### Ingreso familias 2011
```{r}
union_reg <- data.frame()
for (re in unique(casen2011$region)) {
  
tabla_casen <- filter(casen2011, casen2011$region == re)

fn_prom <- function(n){

ingresoh<-switch(n,"ytrabhaj")

tabla_prom_reg <- data.frame()

for (i in unique(tabla_casen$comuna)) {
  casen_fh <- filter(tabla_casen , tabla_casen$comuna == i)
...
union_reg <- rbind(union_reg, calculo_de_2011_1 )
}
# colnames(union_reg) <- c("comuna","region","ytotcorh","yautcorh","ytrabajocorh","yoprcorh")
union_reg
```
...
```{r}
filtro <- filter(union_reg, union_reg$region =="Región de Valparaíso" )
mean(filtro$promedio)
```
...
union_comuna <- data.frame()
for (co in unique(tabla_casen$comuna)) {
  

casen_fh <- filter(tabla_casen, tabla_casen$comuna == co) 
 
casen_fh <- casen_fh[,c("comuna","folio","ytrabhaj")]
casen_fh$folio <- as.character(casen_fh$folio)
...
union_comuna <- rbind(union_comuna,tabla_reg)
}
union_reg <- rbind(union_reg, union_comuna)
}
union_reg
```
...
### Tabla de familias 2011-2020
 
```{r, warning = FALSE, message=FALSE}

familias <- rbind(familias_2,familias_3,familias_4,familias_5) #familias_1,
# write_xlsx(familias,"familias.xlsx")
familias <- read_xlsx("familias.xlsx")
...
### Tabla de personas 2011-2020

```{r, warning = FALSE, message=FALSE} 
personas <- rbind(personas_2,personas_3,personas_4,personas_5) #personas_1,
# write_xlsx(personas,"personas.xlsx")
personas <- read_xlsx("personas.xlsx")
...
```{r}
casen2011_rm <- filter(casen2011, casen2011$region == "Región Metropolitana de Santiago")
...
fu_ingresos <- function(n){
  ...
  nfolio<-switch(n,"folio","folio","folio","folio","folio")
  nregion<-switch(n,"Región Metropolitana de Santiago","Región Metropolitana de Santiago","Región Metropolitana de Santiago","Región Metropolitana de Santiago","Región Metropolitana de Santiago")
...
fn_prom <- function(n){
  ...
  nfolio<-switch(n,"FOLIO","folio","folio","folio","folio","folio")#"folio","folio",
